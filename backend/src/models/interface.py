...
from libs.helpers import get_file_content, get_wg_private_key, get_wg_public_key, ip_range_to_ips, optimalize_ip_range
from models.base import BaseDBModel, Connection
...
class InterfaceTemplateUpdate(BaseModel):
    ...
    ip_range: Optional[str] = Field(None, max_length=255)
...
class InterfaceTemplateDB(BaseDBModel):
    class Meta:
        db_table = f'{DATABASE_WIREGUARD_SCHEMA}"."server_template'
...
class InterfaceDB(BaseDBModel):
    class Meta:
        db_table = f'{DATABASE_WIREGUARD_SCHEMA}"."server_interface'
        PYDANTIC_CLASS = Interface
        DEFAULT_SORT_BY: str = 'f.id'
        sub_sql = f'LEFT JOIN "{DATABASE_WIREGUARD_SCHEMA}"."server_template" st USING("id")'
        sub_columns = 'st.*'
